...
const Myproducts = () => {
  ...
  const handleDelete = (_id, product) => {
    const confirmation = window.confirm(
      `If you delete, you will unable to recover, agree?`
    );
    if (confirmation) {
      ...
      <div className="overflow-x-auto">
        {addedProducts?.length ? (
          <table className="table table-compact w-full">
            ...
            <tbody>
              {addedProducts?.map((post, index) => (
                <tr key={post._id} index={index}>
                  <th>{index + 1}</th>
                  ...
                  <td>
                    {post.location} | {post.locationDetails}
                  </td>
                  <td>{post.level}</td>
          ...
          <p className="text-xl">You have no Post</p>
        )}
      </div>
    </div>
  );
