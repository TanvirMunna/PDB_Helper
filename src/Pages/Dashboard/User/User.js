...
const User = () => {
  ...
  const handleDelete = (_id) => {
    ...
    if (confirmation) {
      fetch(`https://smart-resale-stall-server.vercel.app/sellers/${_id}`, {
        method: "DELETE",
      })
        .then((res) => res.json())
        .then((data) => {
  ...
  const handleVeryfied = (_id) => {
    fetch(
      `https://smart-resale-stall-server.vercel.app/addedProducts/verified/${_id}`,
      {
        method: "PUT",
        headers: {
  ...
  return (
    <div>
      <h1 className="text-3xl my-3">User</h1>
          ...
          <tbody>
            {addedProducts?.map((product, index) => (
              <tr key={product._id} index={index}>
                ...
                <td>
                  {product.role !== "verified" && (
                    <button
                      onClick={() => handleVeryfied(product._id)}
