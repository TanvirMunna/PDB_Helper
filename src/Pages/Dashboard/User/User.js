...
const User = () => {
  ...
  const { data: users, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      const filteredUsers = data.filter(user => user.userType === "user");
          ...
          <tbody>
            {users?.map((user, index) => (
              <tr key={index} index={index}>
                ...
                <td className="hover:cursor-pointer flex align-item-centers">
                  <svg
                  ...
                  <a href={`mailtos:${user.email}`}>
                    <img className="h-[40px] w-[40px]" src="./email.png"/>
                  </a>
                </td>
              </tr>
            ))}
